...
export default function InsightsPage() {
  ...
  return (
    <div className="space-y-6 p-2">
      ...
      {/* Month Detail Panel (choose a month in the chart) */}
      <Card className="p-6">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
              {selectedMonth ? `Details for ${selectedMonth}` : 'Select a month'}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Click a month on the spending trend chart to view activities, trips, budget, and goal progress.
            </p>
          </div>
          {selectedMonthDetails && (
            <div className="text-right">
              <div className="text-xs text-gray-500 dark:text-gray-400">Budget Goal</div>
              <div className="text-2xl font-semibold text-green-600 dark:text-green-400">
                ${selectedMonthDetails.goal.toLocaleString()}
              </div>
            </div>
          )}
        </div>
        ...
        {selectedMonthDetails ? (
          <>
            ...
            <div className="space-y-3">
              <div className="text-sm font-medium text-gray-700 dark:text-gray-200">Spending</div>
              <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">
                ${selectedMonthDetails.spending.toLocaleString()}
              </div>
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                <div
                  className="bg-blue-600 h-3 rounded-full transition-all duration-300"
                  style={{ width: `${selectedMonthDetails.goalProgress}%` }}
                />
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {selectedMonthDetails.goal > 0
                  ? `${selectedMonthDetails.goalProgress.toFixed(1)}% of $${selectedMonthDetails.goal.toLocaleString()}`
                  : 'Set a budget goal to start tracking progress.'}
              </div>
            </div>

            <div className="space-y-3">
              <div className="text-sm font-medium text-gray-700 dark:text-gray-200">Trips</div>
              <div className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">
                {selectedMonthDetails.tripCount}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400">Trips created this month</div>
              <div className="text-sm font-medium text-gray-700 dark:text-gray-200">Activities</div>
              <div className="text-3xl font-bold text-orange-600 dark:text-orange-400">
                {selectedMonthDetails.activities}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400">Activity items logged</div>
            </div>

            <div className="space-y-3">
              <div className="text-sm font-medium text-gray-700 dark:text-gray-200">Top expense categories</div>
              <div className="space-y-2">
                {selectedMonthDetails.expenseBreakdown.length === 0 ? (
                  <div className="text-sm text-gray-500 dark:text-gray-400">No expenses recorded.</div>
                ) : (
                  selectedMonthDetails.expenseBreakdown.slice(0, 4).map((entry) => (
                    <div key={entry.category} className="flex items-center justify-between">
                      <div className="text-sm text-gray-700 dark:text-gray-200">{entry.category}</div>
                      <div className="text-sm font-semibold text-gray-900 dark:text-white">
                        ${entry.amount.toLocaleString()} ({entry.percentage}%)
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className="mt-6">
            <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">Trips in {selectedMonth}</h4>
            {selectedMonthDetails.tripCount === 0 ? (
              <div className="text-sm text-gray-500 dark:text-gray-400">
                No trips created during this month.
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 dark:border-gray-700">
                      <th className="py-2 text-left font-medium text-gray-700 dark:text-gray-300">Destination</th>
                      <th className="py-2 text-left font-medium text-gray-700 dark:text-gray-300">Cost</th>
                      <th className="py-2 text-left font-medium text-gray-700 dark:text-gray-300">Start Date</th>
                    </tr>
                  </thead>
                  <tbody>
                    {selectedMonthDetails.trips.map((trip) => (
                      <tr key={trip.id} className="border-b border-gray-100 dark:border-gray-800">
                        <td className="py-2 text-gray-900 dark:text-white">{trip.userSelection?.destination || 'Unknown'}</td>
                        <td className="py-2 text-gray-700 dark:text-gray-300">${Number(trip.userSelection?.budget || 0).toLocaleString()}</td>
                        <td className="py-2 text-gray-700 dark:text-gray-300">
                          {trip.userSelection?.startDate || (trip.createdAt?.toDate ? trip.createdAt.toDate().toISOString().slice(0, 10) : '')}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="mt-4 text-sm text-gray-500 dark:text-gray-400">
            Click a point on the spending chart to see monthly details.
          </div>
        )}
      </Card>
