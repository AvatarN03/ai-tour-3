...
export default function Auth() {
  ...
  useEffect(() => {
    if (user) router.push(continueTo || "/dashboard");
  ...
  const handleEmailAuth = async (e) => {
    ...
    if (isLogin) {
      const res = await emailSignIn({ email: formData.email, password: formData.password });
  ...
  const handleCheckUsername = async () => {
    ...
    if (!res.success) { toast.error("Could not verify username. Try again."); return; }
    if (res.taken) {
  ...
  const toggleMode = () => {
    setIsLogin(!isLogin);
    setError("");
    setFormData({ name: "", email: "", password: "", username: "", avatarUrl: "", preferences: [] });
  };
          ...
          >
            <span className="text-gray-700 dark:text-gray-300">
              {formData.preferences.length > 0
                ? `${formData.preferences.length} interest${formData.preferences.length !== 1 ? "s" : ""} selected`
    ...
    <div className="space-y-5">
      ...
      {!isLogin && (
        <>
          ...
          <PreferencesSection isMobile={idSuffix === "-mobile"} />
        </>
      )}
    </div>
  );
      ...
      <div className="hidden lg:flex max-w-4xl w-full max-h-[80vh] bg-slate-50 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-700">
        ...
        <div className="flex-1 bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600 p-12 flex flex-col justify-center items-center text-white relative overflow-hidden">
          ...
          <div className="relative z-10 text-center max-w-md">
            <Link href="/" className="w-24 h-24 bg-white/20 rounded-3xl mx-auto mb-8 flex items-center justify-center backdrop-blur-md shadow-lg rotate-3 hover:rotate-6 transition-transform">
            ...
            <p className="text-purple-100 text-lg mb-8">
              {isLogin ? "We've missed you. Sign in to continue your journey." : "Let's get you started on an amazing journey."}
            </p>
            {["Personalized experiences", "Secure and private", "Join thousands of users"].map((text, i) => (
              <div key={i} className="flex items-center gap-3 text-purple-100 text-sm mb-4">
        ...
        <div className="flex-1 p-12 overflow-y-auto scrollbar-gradient">
          ...
          {GoogleButton()}
          {ModeSwitcher()}
          <Link href="/" className="mt-4 inline-block text-sm text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 hover:underline">
            ← Back to Home
          </Link>
        </div>
      </div>
        ...
        <div className="bg-white/95 dark:bg-gray-900/80 backdrop-blur-xl rounded-xl shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-700">
          <div className="bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600 px-4 py-8 text-center relative overflow-hidden">
            ...
            <div className="relative z-10 flex justify-between items-center gap-2 w-full">
              <Link href="/" className="w-20 h-20 bg-white/20 rounded-2xl mx-auto mb-4 flex items-center justify-center backdrop-blur-md shadow-lg rotate-3 hover:rotate-6 transition-transform">
